    ...
    <>
      <section className="py-16 text-center" style={{ background: 'radial-gradient(ellipse at 50% 100%, rgba(212,169,64,0.06) 0%, transparent 60%), var(--c-surface)', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        <div className="max-w-3xl mx-auto px-4">
          ...
          <h1 className="font-serif text-4xl font-bold text-white mb-4">Careers at El Rachum Auctions</h1>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-16">
            {perks.map(({ icon, label, desc }) => (
              <div key={label} className="p-5 rounded-2xl text-center" style={{ background: 'var(--c-card)', border: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <h2 className="font-serif text-3xl font-bold text-white mb-8">Current <span style={{ color: 'var(--c-gold)' }}>Openings</span></h2>

          <div className="space-y-4 mb-12">
            {roles.map(({ title, type, desc, reqs }) => (
              <div key={title} className="p-6 rounded-2xl" style={{ background: 'var(--c-card)', border: '1px solid rgba(255,255,255,0.07)' }}>
                <div className="flex flex-wrap items-center gap-3 mb-3">
                  <h3 className="text-base font-semibold text-white">{title}</h3>
                  <span className="px-2.5 py-0.5 rounded-full text-xs font-medium" style={{ background: 'rgba(192,57,43,0.15)', color: 'var(--c-accent)', border: '1px solid rgba(192,57,43,0.3)' }}>{type}</span>
                </div>
                <p className="text-sm leading-relaxed mb-3" style={{ color: 'var(--c-muted)' }}>{desc}</p>
                <ul className="flex flex-wrap gap-2">
                  {reqs.map(r => (
                    <li key={r} className="text-xs px-3 py-1 rounded-lg" style={{ background: 'rgba(255,255,255,0.05)', color: 'var(--c-muted)', border: '1px solid rgba(255,255,255,0.08)' }}>{r}</li>
                  ))}
                </ul>
              </div>
            ))}
          ...
          <div className="p-8 rounded-2xl text-center" style={{ background: 'var(--c-card)', border: '1px solid rgba(255,255,255,0.07)' }}>
            <p className="text-xs font-semibold uppercase tracking-[0.15em] mb-2" style={{ color: 'var(--c-gold)' }}>Don't see your role?</p>
            <h3 className="font-serif text-2xl font-bold text-white mb-3">Send an Open Application</h3>
